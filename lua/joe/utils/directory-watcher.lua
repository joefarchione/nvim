...
local debounce = function(fn, delay)
  return function(...)
    local args = { ... }
    if debounce_timer then debounce_timer:close() end
...
-- creating duplicates
M.registerOnChangeHandler = function(name, handler) on_change_handlers[name] = handler end
...
M.setup = function(opts)
  ...
  if watcher then M.stop() end

  -- Create fs_event handle
  local fs_event = uv.new_fs_event()
  if not fs_event then return false end
